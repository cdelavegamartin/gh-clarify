...
`.github-workflow-template/code-clarifications.yml` gets this prompt body by
running `gh clarify prompt` (which reads this file). Copilot app users can
instead paste the prompt body below into `save_workflow` and use the
...
This prompt text orchestrates the `gh clarify` extension subcommands — the
agent should call those for every `gh`/GraphQL interaction and focus only on
judgment work (reading code, classifying patterns, writing answer text).
Requires `gh extension install cdelavegamartin/gh-clarify` to have already
run in this environment so `gh clarify` is on `PATH`.
...
The `gh clarify` extension must already be installed (`gh extension install
cdelavegamartin/gh-clarify`). All its subcommands print their `--help` text
when invoked with `--help` or `-h`.

0. Guard: run `gh clarify check-enabled --require`.
...
1. New questions (no bot answer yet):
   ...
   b. For each discussion in the list:
      i.   Read the `body` field (see `templates/question-template.md` for the
           expected shape): file path, line range, commit SHA, permalink,
      ...
      iv.  Classify it as a common pattern, an anti-pattern, a repo-specific
           ...
           high confidence.
      v.   Write the answer using `templates/answer-template.md`, filling in
           the model id you are running as and including a confidence rating
      ...
      vi.  Post the answer by calling
           `gh clarify post-answer <discussion-number> <body-file>`
...
2. Follow-up requests (already answered, but with new human replies since):
   ...
   b. For each discussion in the list:
      ...
      ii.  Judge whether the last comment is from a human (not the bot's
           ...
           This judgment call is yours — the command does not make it for you.
      iii. If yes: write a deeper answer using `templates/followup-template.md`
           (minimal working example + more detailed explanation + confidence
           rating), save it to a temporary file, and post it by calling
           `gh clarify post-answer <discussion-number> <body-file>`
           Do NOT call `gh clarify mark-answered` again for follow-ups — the
...
3. Do not create, edit, or close discussions beyond what's described above.
   ...
   discussion comments/labels via `gh clarify`.
````
